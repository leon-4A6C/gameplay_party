...
-- Thu 18 Oct 2018 07:10:47 PM CEST
...
CREATE TABLE IF NOT EXISTS `gpp`.`betalingen` (
  `betaal_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `percentage_betaald` INT NOT NULL,
  `datum` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `betaalmethode` VARCHAR(50) NOT NULL,
  `reservering_reservering_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`betaal_id`),
  INDEX `fk_betalingen_reservering1_idx` (`reservering_reservering_id` ASC),
  CONSTRAINT `fk_betalingen_reservering1`
    FOREIGN KEY (`reservering_reservering_id`)
    REFERENCES `gpp`.`reservering` (`reservering_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
